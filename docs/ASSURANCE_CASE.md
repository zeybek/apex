...
- **R1 — Instructions only.** The installed package (`plugins/apex/`) is Markdown and JSON. It contains no executable code, makes no network calls, and reads no credentials. Any action taken while a skill is loaded is taken by the host agent under the host's own permissions.
...
|---|---|---|---|
| Package content → host agent | `plugins/apex/**` Markdown and JSON | Host agent, user's repository, model | R1: no code. R2: validator injection scan + human review of every skill change. |
...
- **Economy of mechanism / least functionality.** The product is text. The only executables are two validators and an eval harness, none of which ship to users (R1, R5).
...
|---|---|---|
| Prompt injection via instruction content (OWASP LLM01) | Highest: the product is instructions | Validator scan for override/exfiltration phrasing; explicit review checklist item; `SECURITY.md` documents the limits of the scan. |
...
- `scripts/validate_package.py` — injection and secret signatures, link and size limits; `tests/test_validate_package.py`.
