...
When a `.apex-design/<slug>/` planning workspace covers the change, review the diff against its `design.md`, `requirements.md`, and `glossary.md` in addition to the standard risk-first checks, and flag where the implementation diverges from the recorded decisions, fails an acceptance scenario, or drifts from the agreed vocabulary.
